...
import sql_veggie_queries, sql_birthday_queries, sql_professional_recipes_queries, \
       sql_easy_recipes_queries, sql_holiday_queries, sql_romantic_queries, sql_drink_queries
...
def cocktail():
    if request.method == 'GET':
        ...
        drinks, snacks = sql_drink_queries.get_drink_results_by_params(alcoholic, main_ingredient,
                                                                       preferable_glasses, max_ingredients,
                                                                       side_dish)
        if not drinks:
            print("drinks result is empty")

        return render_template('results/cocktail_results.html', drinks=drinks, snacks=snacks)
    else:
        return 'failed to load page or to send request'


if __name__ == "__main__":
    app.run(host='0.0.0.0', port='40102', threaded=True)
