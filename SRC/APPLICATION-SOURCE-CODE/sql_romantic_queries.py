...
def get_recipe_from_db_by_romantic_meal_filter(main_ingredient, side_ingredient, dessert_ingredient,
                                               max_prep_time_in_sec,
                                               conn):
    ...
    try:
        query = "SELECT DISTINCT sides.recipe_id AS side_recipe_id, mains.recipe_id AS main_recipe_id," \
                ...
                ("AND (" + get_query_of_recipes_like_keywords(side_ingredient, "ListOfCourses") + ")"
                ...
                " AND (" + get_query_of_recipes_like_keywords(main_ingredient, "ListOfCourses") + ")) AS mains, " \
                "(SELECT DISTINCT Recipe.recipe_id, prep_time FROM Recipe, ListOfCourses " \
        ...
        if dessert_ingredient.lower() == "nosugar":
            ...
            query += (" AND (" + get_query_of_recipes_like_keywords(dessert_ingredient, "desserts") + ") ")
        else:
            query += " AND EXISTS (SELECT DISTINCT recipe_id FROM ListOfIngredients " \
                     "WHERE ListOfIngredients.recipe_id = desserts.recipe_id AND " \
...
def get_query_of_recipes_like_keywords(main_ingredient, table):

    res = ""
    base_query = ("%s" % table) + ".recipe_id LIKE '%"
    keywords = []
