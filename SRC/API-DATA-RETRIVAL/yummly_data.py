...
def insert_from_metadata_api_to_db(metadata_name, table_name, col_name, file_name, conn):
...
def write_json_recipes_to_db_from_file(file_name, cuisine, conn):
    with open(file_name) as f:
        ...
        for match in recipes_by_cuisine['matches']:
            ...
            mysql_queries.insert_multi_data_to_db("ListOfCuisines", "Cuisines", recipe_cuisines,
                                                  recipe_id, "cuisine_name", conn)
            mysql_queries.insert_multi_data_to_db("ListOfIngredients", "Ingredients", ingredients,
                                                  recipe_id, "ingredient_name", conn)
            if 'holiday' in match['attributes']:
                holidays = match['attributes']['holiday']
                mysql_queries.insert_multi_data_to_db("ListOfHolidays", "Holidays", holidays,
                                                      recipe_id, "holiday_name", conn)
            if 'course' in match['attributes']:
                courses = match['attributes']['course']
                mysql_queries.insert_multi_data_to_db("ListOfCourses", "Courses", courses,
                                                      recipe_id, "course_name", conn)


def insert_recipe(recipe_id, conn):
...
def data_retrieve():
    ...
    for cuisine in cuisines:
        ...
        outfile.close()
        write_json_recipes_to_db_from_file(cuisine + "_recipes.json", cuisine, conn)
