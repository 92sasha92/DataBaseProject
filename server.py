...
def cocktail():
    if request.method == 'GET':
        ...
        qry, side_dishes = sql_drink_queries.get_drink_results_by_params(alcoholic, main_ingredient,
                                                                         preferable_glasses, max_ingredients,
                                                                         side_dish)
        print(qry)
        print(side_dishes)
        return render_template('cocktail_results.html', drinks=qry, side_dishes=side_dishes)
    else:
        return 'failed to load page or to send request'
