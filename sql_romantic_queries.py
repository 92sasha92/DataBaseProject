...
def get_recipe_from_db_by_romantic_meal_filter(main_ingredient, side_ingredient, dessert_ingredient, max_prep_time_in_sec,
    ...
    try:
        query = "SELECT DISTINCT sides.recipe_id AS side_recipe_id, mains.recipe_id AS main_recipe_id," \
                ...
                " AND (" + get_query_of_main_recipes_like_keywords(main_ingredient) + ") AND sides.recipe_id IN " \
                "(SELECT recipe_id FROM ListOfIngredients WHERE LOWER(ingredient_name) LIKE '%" + side_ingredient + \
                "%') "
...
def get_query_of_main_recipes_like_keywords(main_ingredient):
    res = ""
